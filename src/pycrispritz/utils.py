...
def exception_handler(
    ...
    Args:
        exception_type (Exception): The type of exception.
        exception (str): The exception message.
        code (int): The error code.
        debug (bool): A flag indicating debug mode. If set, trace full error stack.
...
def retrieve_fasta_files(path: str) -> List[str]:
    ...
    Args:
        path (str): The directory path to search for FASTA files.

    Returns:
        List[str]: A list of paths to FASTA files found in the specified directory.
    """

    return [f for ext in FASTAEXT for f in glob(os.path.join(path, f"*.{ext}"))]
...
def retrieve_vcf_files(path: str) -> List[str]:
    ...
    Args:
        path (str): The directory path to search for VCF files.
...
def read_fasta_header(fastafile: str, debug: bool) -> str:
    """
    Reads the header line from a FASTA file.

    Args:
        fastafile (str): The path to the FASTA file to read the header from.
        debug (bool): A flag indicating debug mode.

    Returns:
        str: The header line extracted from the FASTA file.
    """
    ...
    try:
        ...
        exception_handler(
            IOError,
            f"An error occurred while recovering chromosome name in {fastafile}",
            os.EX_OSFILE,
            debug,
        )
